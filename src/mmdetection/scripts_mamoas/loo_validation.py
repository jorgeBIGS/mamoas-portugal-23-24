...
for data_root in data_folders:
    ...
    for i in range(num_folds):
        ...
        subprocess.run(["python", "src/mmdetection/tools/train.py", config_file, 
        ...
        lista = sorted(glob(f"{work_dir_fold}/*.pth"))
        if len(lista)>0:
            ckpt_file = lista[-1]
        else:
            ckpt_file = None
        ...
        subprocess.run(["python", "src/mmdetection/tools/test.py", 
                        config_file, 
                        f"{ckpt_file}", 
